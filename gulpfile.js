...
gulp.task('watch-server', function () {
	gulp.watch(path.join(__dirname, '/routes/**/*.js'), gulp.series('server'));
	gulp.watch([path.join(__dirname, '/utils/fc_wrangler/*.js')], gulp.series('server'));
	gulp.watch([path.join(__dirname, '/utils/*.js')], gulp.series('server'));
	gulp.watch(path.join(__dirname, '/app.js'), gulp.series('server'));
});
...
// ---------------- Runnable Gulp Tasks ---------------- //
gulp.task('default', gulp.parallel('watch-sass', 'watch-server', 'server'));
gulp.task('marbles_tls', gulp.parallel('env_tls', 'watch-sass', 'watch-server', 'server'));		//run with command `gulp marbles_tls` for IBM Cloud blockchain service
gulp.task('marbles_local', gulp.parallel('env_local', 'watch-sass', 'watch-server', 'server'));	//run with command `gulp marbles_local` for a local network
gulp.task('marbles_cs', gulp.parallel('env_cs', 'watch-sass', 'watch-server', 'server'));		//run with command `gulp marbles_cs` for IBM Cloud container service
gulp.task('marbles_dev', gulp.parallel('env_dev', 'watch-sass', 'watch-server', 'server'));		//run with command `gulp marbles_dev` if you are me
gulp.task('marbles_dev2', gulp.parallel('env_dev2', 'watch-sass', 'watch-server', 'server'));	//run with command `gulp marbles_dev` if you are me
